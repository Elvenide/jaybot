...
/**
 * getFirstDigitsAsDecimal() util imported from Elvenide's discord-arts-zhorde
 * 
 * This method takes a large number like "9360" and converts it to a small decimal like "9.3".
...
function getFirstDigitsAsDecimal(numString) {
  ...
  return `${numString.slice(0, digits)}${decimal == "0" || decimal == "00" || digits == 3 ? "" : `.${decimal.replace(/0$/g, "")}`}`;
}

/**
 * abbreviateNumber() util imported from Elvenide's discord-arts-zhorde
 * 
...
function abbreviateNumber(number) {
  ...
  const abbreviations = ["", "K", "M", "B", "T"].concat(new Array(letters.length).fill("AA").map((_, i) => letters[i].repeat(2)));
